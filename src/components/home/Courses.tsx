            ...
            {courses.map((course, index) => {
              ...
              return (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  className="group bg-white rounded-3xl overflow-hidden border border-slate-100 shadow-[0_10px_30px_rgba(0,0,0,0.05)] hover:shadow-[0_20px_50px_rgba(0,0,0,0.1)] transition-all duration-500 flex flex-col h-full hover:-translate-y-1 relative"
                >
                  {/* Top Image Section */}
                  <div className="h-52 relative overflow-hidden shrink-0">
                    ...
                    <div className="absolute top-6 left-6 z-20 px-5 py-2.5 bg-white text-primary-green text-[10px] font-black uppercase tracking-[0.2em] rounded-xl shadow-xl border border-white/50">
                      {course.name.toUpperCase().includes('ONLINE') ? 'Online' : 'Offline'}
                    </div>
                  </div>
                        ...
                        return (
                          <div className="flex gap-4">
                            <Link href={slug ? `/courses/${slug}` : '/courses'} className="flex-1">
                              <button className="w-full py-3 rounded-xl font-black text-[10px] uppercase tracking-widest text-slate-500 bg-slate-50/50 hover:bg-slate-100 hover:text-dark transition-all duration-300 border border-slate-200">
                                Course Details
                              </button>
                            </Link>
                            <Link href={`/register?course=${encodeURIComponent(course.name)}`} className="flex-1">
                              <button
                                className="w-full py-3 rounded-xl font-black text-[10px] uppercase tracking-widest text-white bg-primary-green hover:bg-dark transition-all duration-300 shadow-lg shadow-primary-green/20"
                              >
                                Register Now
                              </button>
                            </Link>
